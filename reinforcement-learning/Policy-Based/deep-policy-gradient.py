...
class Agent:
    ...
    def initialize_policy_network(self):
        ...
        # tf.placeholder() for variables getting from the environment
        # tf.get_variable() for variables generated by the initializer
        ...
        with tf.name_scope("fully-connected-01"):
            w1 = tf.get_variable(name = "w1",
                shape = [self.state_shape, 10], initializer = initializer)
            b1 = tf.get_variable(name = "b1",
                shape = [10], initializer = initializer)
            f1 = tf.matmul(self.s, w1) + b1

        with tf.name_scope("fully-connected-02"):
            w2 = tf.get_variable(name = "w2",
                shape = [10, 10], initializer = initializer)
        ...
        with tf.name_scope("output"):
            wo = tf.get_variable(name = "wo",
                shape = [10, self.n_actions], initializer = initializer)
            bo = tf.get_variable(name = "bo",
                shape = [self.n_actions], initializer = initializer)
            a_pre_softmax = tf.matmul(f2, wo) + bo
            self.a_ = tf.nn.softmax(a_pre_softmax)
        ...
        with tf.name_scope("loss"):
            ...
            # Compare the real action & the training action
            prob = tf.nn.softmax_cross_entropy_with_logits(
                logits = a_pre_softmax, labels = self.a)
            self.loss = tf.reduce_mean(prob * self.r)
    ...
    def choose_action(self, s):
        policy = self.sess.run(self.a_, feed_dict = {self.s: [s]})[0]
